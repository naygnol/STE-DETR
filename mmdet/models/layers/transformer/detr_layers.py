...
class DetrTransformerDecoderLayer(BaseModule):
    ...
    def _init_layers(self) -> None:
        """Initialize self-attention, FFN, and normalization."""
        self.self_attn = MultiheadAttention(**self.self_attn_cfg)
        self.cross_attn_cls = MultiheadAttention(**self.cross_attn_cfg)
        self.cross_attn_box = MultiheadAttention(**self.cross_attn_cfg)
        self.embed_dims = self.self_attn.embed_dims
        self.ffn_cls = FFN(**self.ffn_cfg)
        self.ffn_box = FFN(**self.ffn_cfg)
        norms_list = [
            build_norm_layer(self.norm_cfg, self.embed_dims)[1]
            for _ in range(5)
        ]
    ...
    def forward(self,
        ...
        query = self.norms[0](query)
        query_cls, x, y = self.cross_attn_cls(
            query=query,
            key=key,
            value=value,
            query_pos=query_pos,
            ...
            key_padding_mask=key_padding_mask,
            **kwargs)
        query_cls = self.norms[1](query_cls)
        query_cls = self.ffn_cls(query_cls)
        query_cls = self.norms[2](query_cls)

        query_box, x, y = self.cross_attn_box(
            query=query,
            key=key,
            value=value,
            query_pos=query_pos,
            ...
            key_padding_mask=key_padding_mask,
            **kwargs)
        query_box = self.norms[3](query_box)
        query_box = self.ffn_box(query_box)
        query_box = self.norms[4](query_box)

        return query_cls, query_box

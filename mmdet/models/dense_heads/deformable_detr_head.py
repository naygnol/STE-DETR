...
class DeformableDETRHead(DETRHead):
    ...
    def forward(self, hidden_states_cls: Tensor, hidden_states_box: Tensor,
        ...
        for layer_id in range(hidden_states_cls.shape[0]):
            reference = inverse_sigmoid(references[layer_id])
            # NOTE The last reference will not be used.
            # hidden_state = hidden_states[layer_id]
            outputs_class = self.cls_branches[layer_id](hidden_states_cls[layer_id])
            tmp_reg_preds = self.reg_branches[layer_id](hidden_states_box[layer_id])
    ...
    def predict(self,
                hidden_states_cls: Tensor,
                hidden_states_box: Tensor,
                references: List[Tensor],
                rego_match: List[dict] = None,
                batch_data_samples: SampleList = None,
                rescale: bool = True, type: int = None) -> InstanceList:
        """Perform forward propagation and loss calculation of the detection
        head on the queries of the upstream network.
        ...
        predictions = self.predict_by_feat(
            rego_match[0][0]['pred_logits_rego_0_match'], rego_match[0][1]['pred_boxes_rego_0_match'], batch_img_metas=batch_img_metas, rescale=rescale)
        return predictions
    ...
    def predict_by_feat(self,
        ...
        Returns:
            ...
            after the post process.
        """
        cls_scores = all_layers_cls_scores
        bbox_preds = all_layers_bbox_preds

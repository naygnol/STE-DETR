...
class DETRHead(BaseModule):
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg, indices_list) = cls_reg_targets
        ...
        for i in range(len(batch_gt_instances)):
            ...
            size = None
            size = batch_gt_instances[i].bboxes.new_tensor([img_w, img_h, img_w, img_h]).unsqueeze(0).repeat(batch_gt_instances[i].bboxes.size(0), 1)
            target = {}
            target['labels'] = batch_gt_instances[i].labels
            target['boxes'] = batch_gt_instances[i].bboxes
            target['boxes'] = target['boxes'] / size
            target['boxes'] = bbox_xyxy_to_cxcywh(target['boxes'])
            targets.append(target)
        target_boxes = torch.cat([t["boxes"][v[1]] for t, v in zip(targets, indices_list)], dim=0)
        target_classes_o = torch.cat([t["labels"][v[1]]
                                      for t, v in zip(targets, indices_list)])
        pos_idx = _get_src_permutation_idx(indices_list)
        pos_idx_c = pos_idx + (target_classes_o.cpu(),)
        pred_boxes = torch.stack(bbox_preds_list)
        src_logits = torch.stack(cls_scores_list)
        src_boxes = pred_boxes[pos_idx]
        cls_avg_factor = cls_avg_factor / get_world_size()
        if isinstance(cls_avg_factor, float):
            gpu_index = pred_boxes.get_device()
            cls_avg_factor = torch.tensor([cls_avg_factor])
            cls_avg_factor = cls_avg_factor.to(device=f"cuda:{gpu_index}")
        # num_boxes = torch.clamp(cls_avg_factor / get_world_size(), min=1).item()
        num_boxes = torch.clamp(cls_avg_factor, min=1).item()
        loss_cls, loc_weight = IA_BCE_loss(src_logits, pos_idx_c, src_boxes,
                                        target_boxes, indices_list, num_boxes,
                                        0.25, 2.0,
                                        1,)
    ...
    def get_targets(self, cls_scores_list: List[Tensor],
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         pos_inds_list,
         neg_inds_list, indices_list) = multi_apply(self._get_targets_single,
                                      cls_scores_list, bbox_preds_list,
                                      batch_gt_instances, batch_img_metas)
        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))
        return (labels_list, label_weights_list, bbox_targets_list,
                bbox_weights_list, num_total_pos, num_total_neg, indices_list)
    ...
    def _get_targets_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        assign_result, indices = self.assigner.assign(
...
def _get_src_permutation_idx(indices, pos=True):
...
def IA_BCE_loss(src_logits, pos_idx_c, src_boxes, target_boxes, indices, avg_factor, alpha, gamma, w_prime=1, ):
    ...
    iou_scores = torch.diag(box_iou(bbox_cxcywh_to_xyxy(src_boxes), bbox_cxcywh_to_xyxy(target_boxes))[0])
